...
/* ── จำนวนทศนิยมที่แสดงบนจอ ────────────────────────────────────────────────
   **ทั้งระบบใช้ 3 ตำแหน่ง** — ให้ตรงกับ `_DP = 3` ใน `shared.py` ที่ใช้ปัด
   ตอนตัดสิน OK/NG จริง · ถ้าจะเปลี่ยนต้องเปลี่ยนคู่กันทั้ง 2 ที่เสมอ ไม่งั้น
   ค่าบนจอกับค่าที่ใช้ตัดสินจะคนละตัวแล้วอธิบายผลให้ operator ไม่ได้

   `DP_MM`  ขนาดชิ้นงาน / nominal / tolerance
            ⚠ เดิมตั้งไว้ 2 เพราะเข้าใจว่า TM-X ออกแค่ 2 ตำแหน่ง — **ไม่จริง**
              ของจริงที่ส่งมาทาง FTP เป็น 3 ตำแหน่ง (เช่น `8.157`) การตัดเหลือ
              2 จึงกลืนหลักสุดท้ายที่เครื่องวัดได้จริงทิ้งไปทุกแถว
...
export const DP_MM  = 3;
export const DP_OFF = 3;
