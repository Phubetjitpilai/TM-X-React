...
# URL ที่เอาไปแปะใน Power BI — ต้องเป็น IP/ชื่อเครื่องที่ **เครื่องอื่นเรียกถึงได้**
# ไม่ใช่ 127.0.0.1 · ถ้า IP เครื่องเปลี่ยน (DHCP) ลิงก์รูปใน Power BI จะพังทั้งหมด
# โดยไม่มีอะไรเตือน → ขอ IT ตั้ง static IP หรือ DNS name ให้เครื่องนี้
PUBLIC_BASE_URL = os.getenv("IMAGE_PUBLIC_BASE_URL", f"http://127.0.0.1:{SERVER_PORT}")
...
if __name__ == "__main__":
    ...
    if PUBLIC_BASE_URL.startswith(("http://127.0.0.1", "http://localhost")):
        ...
        log.warning("    ตั้ง IMAGE_PUBLIC_BASE_URL ใน .env เป็น IP จริงของเครื่องนี้")
    log.info("=" * 66)
